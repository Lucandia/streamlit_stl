...
class STLComponent:
    ...
    def stl_from_text(
        ...
        text: str,
        color: str = "#696969",
        material: Literal["material", "wireframe"] = "material",
        ...
        **kwargs,
        ...
        ----------
        text : str
            Text of the stl file
        color : str, default "#696969"
            The hexadecimal color (starting with '#')
        material : Literal["material", "wireframe"], default "material"
            The material of the stl file ('material' or 'wireframe')
        height : int, default 500
            Height of the component display
        """
        ...
        if material not in ("material", "wireframe"):
            raise ValueError(
                f'The possible materials are "material" or "wireframe", got {material} instead'
            )
        if color[0] != "#":
            raise ValueError(
                f"The color must be a hexadecimal value starting with '#', got {color} instead"
            )
        if text is not None:
            try:
                # Create a temporary file in the temporary stl folder
                with tempfile.NamedTemporaryFile(
                    dir=self.temp_folder, suffix=".stl", delete=False
                ) as temp_file:
                    ...
                    file_path = temp_file.name.split(os.sep)[
                        -1
                    ]  # Store the relative path
                    self.current_temp_files.append(
                        temp_file.name
    ...
    def cleanup_temp_files(self):
        ...
        try:
            ...
            print(
                f"Error deleting temporary streamlit-stl folder {self.temp_folder}: {e}"
            )
